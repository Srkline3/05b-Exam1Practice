...
def problem2a(circle, rectangle, window):
    ...
    window.render()
    window.continue_on_mouse_click()
    line1start = rectangle.get_upper_right_corner()
    line1end = rectangle.get_lower_left_corner()
...
def problem2b(rect, n, delta, win):
    ...
    for k in range(n):
        rect.attach_to(win)
        rect = rg.Rectangle(rg.Point(rect.get_upper_right_corner().x + delta, rect.get_upper_right_corner().y - delta), rg.Point(rect.get_lower_left_corner().x - delta, rect.get_lower_left_corner().y + delta))
    win.render()
